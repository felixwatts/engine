...
public class SpatialIndex<T>
{
	...
	private final Hashtable<MutableInteger, Array<T>> _cells = new Hashtable<MutableInteger, Array<T>>();
	private final Hashtable<Object, ItemDetails> _contents = new Hashtable<Object, ItemDetails>();
	private final Array<T> _result = new Array<T>();
	private final MutableInteger _lookupKey = new MutableInteger(0);
	...
	public void put(T obj, float x, float y, int z, float w, float h)
	{
		...
		for(int cy = toCell(y); cy <= toCell(y + h); cy++)
		{
			for(int cx = toCell(x); cx <= toCell(x + w); cx++)
			{
				...
				{
					cell = new Array<T>();
					_cells.put(new MutableInteger(_lookupKey.value()), cell);
				}
	...
	{
		_result.clear();
		Enumeration<Object> keys = _contents.keys();
		while(keys.hasMoreElements())
			_result.add((T)keys.nextElement());
